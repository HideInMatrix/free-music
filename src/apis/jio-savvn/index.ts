...
export const getArtistDetail = async (
  ...
  const resp =  await fetchArtistsById({ id: artistId, options })
  return resp || {id: "", name: "", image: "", topSongs: []}
}
...
export const getArtistSongsById = async (
  ...
  const resp = await fetchSongsByArtists({ id: artistId, options })
  return resp.data
}
...
export const getArtistAlbumsById = async (
  ...
  const resp = await fetchAlbumsByArtistIdFn({ id: artistId, page: 0, options })
  return resp.data
}